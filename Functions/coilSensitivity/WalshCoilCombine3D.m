...
if ~isfield(param, 'fil')
    param.fil = [3,3,1];
elseif min(param.fil)<1
    param.fil = [3,3,1];
    warning('param.fil is set to [3,3,1]');
end
...
%             S(j,i,k,:) = reshape(abs(U(:,end)).^1.5 .* (U(:,end)./abs(U(:,end))), [1 1 1 Nc]);   
...
%     F = F/F(1);
%     slicer(abs(F),1,1,'');
%     sc = bsxfun(@times, sc, F);
%     xc = bsxfun(@times, xc, 1./F);

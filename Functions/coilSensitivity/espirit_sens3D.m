...
% eigThresh_1 = 0.01; % Larger value gives smoother sensitivity maps; Default: 0.02

% threshold of eigen vector decomposition in image space. 
% eigThresh_2 = 0.9; % Larger value gives more localized maps; Defalut: 0.95
...
% paper. 
if isempty(idx)
    idx = ceil(numel(S(:))*param.eSRTsig); 
end
disp([num2str(100*double(idx)/numel(S(:))) '% of singular vectors are included.']);
if ( double(idx)/numel(S(:)) >= 0.7 || double(idx)/numel(S(:)) <= 0.2 || double(idx) < max(find(S >= S(1)*0.01)) )
...
if not(isempty(param.fixeSRTsig))
    idx = ceil(numel(S(:))*param.fixeSRTsig);
     warning('Debugging, sigular value threshold is fexed!!!!!!');
end

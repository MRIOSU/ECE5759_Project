...
if noiseScan > 0
    ...
    noise_data = zeros([size(noise.data{1}), noiseScan]);
    for noise_idx = 1:noiseScan
        noise_data(:,:,noise_idx) = noise.data{noise_idx};
    end    
    noise_dwelltime_us = noise.head.sample_time_us(end);
else
    noise_data = [];
    noise_dwelltime_us = [];
end
...
param.noise = noise_data;
param.noise_dwelltime_us = noise_dwelltime_us;
...
%% Read the K-space
param.acquisition_dwelltime_us = meas.head.sample_time_us(end);
